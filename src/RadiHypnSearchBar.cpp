...
RadiHypnSearchBar::RadiHypnSearchBar()
    : Gtk::Box(Gtk::ORIENTATION_HORIZONTAL), m_entry(),
      m_button_search("Search"), m_button_clear("Clear"),
  ...
  m_entry.signal_changed().connect(
      sigc::mem_fun(*this, &RadiHypnSearchBar::onEntryChanged));
  m_button_search.signal_clicked().connect(
      sigc::mem_fun(*this, &RadiHypnSearchBar::onSearchClicked));
  m_button_clear.signal_clicked().connect(
      sigc::mem_fun(*this, &RadiHypnSearchBar::onClearClicked));
  m_entry.signal_activate().connect(
      sigc::mem_fun(*this, &RadiHypnSearchBar::onSearchClicked));
  m_checkbox.signal_clicked().connect(
      sigc::mem_fun(*this, &RadiHypnSearchBar::onSearchClicked));
}

sigc::signal<void(std::string, bool)>
RadiHypnSearchBar::signal_search_started() {
